data = xlsread('ArquivosDeEntrada/annealing13_norm_ext.xls'); % Carrega os dados normalizados a partir do arquivo dadosirisnorm.xls
...
top = [3]; % Topologia da rede (quantidade de camadas ocultas e quantos neuronios existem em cada camada)
...
for i = percentual % para a taxa de treinamento indo de 10 a 80 por cento
    ...
    for j = 1:20 % executar 20 vezes
        j
        [x,d,x_teste,d_teste] = geraDadosAnnealing(data,i); % Separa os dados para treinamento e teste
...
xlswrite('dadosGerados/MLP_Annealing_Taxa_de_Acerto',acertos); % Salva a matriz final de Taxas de acertos com os critérios pedidos em um arquivo .xls
xlswrite('dadosGerados/MLP_Annealing_Matriz_de_Confusão',matrizconfusao); % Salva a matriz final de confusões em um arquivo .xls
